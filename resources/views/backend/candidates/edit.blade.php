        ...
        <div class="container mt-4">
            ...
            <form method="POST" action="{{ route('candidates.update', $candidate->id) }}">
                ...
                <div class="mb-3">
                    ...
                    <input value="@if(old('full_name')){{old('full_name')}}@elseif(empty(old('full_name')) && old('_token')) {{''}}@else{{Arr::get($candidate,'full_name')}}@endif" 
                        type="text" 
                        class="form-control" 
                        name="full_name" 
                ...
                <div class="mb-3">
                    ...
                    <input type="text" readonly autocomplete="off" class="form-control" id="email" placeholder="Enter email" name="email" maxlength="50" value="@if(old('email')){{old('email')}}@elseif(empty(old('email')) && old('_token')) {{''}}@else{{Arr::get($candidate,'email')}}@endif">
                </div>

                <div class="mb-3">
                    <label for="Country" class="form-label">Country</label>
                    <select class="form-control c-select" id="country" disabled  name="country_code" rel="{{Arr::get($candidate, 'country_code')}}" >
                        @if(!empty($countries) )
                ...
                <div class="mb-3">
                    ...
                    <input type="text" autocomplete="off" readonly class="form-control" id="mobile_number" placeholder="Mobile Number" name="mobile_number" maxlength="18" value="@if(old('mobile_number')){{old('mobile_number')}}@elseif(empty(old('mobile_number')) && old('_token')) {{''}}@else{{Arr::get($candidate,'mobile_number')}}@endif">
                </div>

                <div class="mb-3">
                    <label for="Mobile" class="form-label">Area of Interest</label>
                    <select class="form-control c-select" id="area_of_interest_option_id" disabled  name="area_of_interest_option_id" rel="{{old('area_of_interest_option_id')}}" >
                        ...
                        @if(!empty($areaOfInterests) )
                            ...
                            @foreach($areaOfInterests as $object)
                                ...
                                @elseif($object['id']==(Arr::get($candidate, 'area_of_interest_option_id')) && empty(old('area_of_interest_option_id')) )
                                    <option value="{{ trim($object['id']) }}" selected>{{trim($object['name'])}} </option>   
                                @else
