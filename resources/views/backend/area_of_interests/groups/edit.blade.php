...
@section('content')
    <div class="bg-light p-4 rounded">
        ...
        <div class="container mt-4">
            ...
            <form method="POST" action="{{ route('area_of_interest_groups.update', $areaOfInterestGroup->id) }}">
                @method('patch')
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">Name</label>
                    <input value="@if(old('name')){{old('name')}}@elseif(empty(old('name')) && old('_token')) {{''}}@else{{Arr::get($areaOfInterestGroup,'name')}}@endif" 
                        type="text" 
                        class="form-control" 
                        name="name" 
                        placeholder="Name">
                        ...
                        @if(!empty($booleanOptions))
                            @foreach($booleanOptions as $key=>$value)
                                ...
                                @if((old('_token') && old('is_enabled') == $key)) 
                                    {{'checked'}}
                                @elseif(old('_token') == null && Arr::get($areaOfInterestGroup, 'is_enabled'))  
                                    {{ Arr::get($areaOfInterestGroup, 'is_enabled') == $key ? 'checked' : '' }}         
                                @else 
